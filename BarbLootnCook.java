...
import LootnCook.tasks.Cook;
import LootnCook.tasks.Drop;
import LootnCook.tasks.Loot;
...
@Script.Manifest(name = "Dans Barbarian Fish Cooker",
        description = "Picks up and Cooks Fish on fire",
        properties = "client=4; topic=999; author=danny;")
...
tasklist.add(new Cook(ctx));
    ...
    public void repaint(Graphics graphics) {
        ...
        g.fillRect(0,0,150,100); //draws rectangle


        g.setColor(new Color(255,255,255)); //draws white border
        g.drawRect(0,0,150,100); //same as first

        g.drawString("Dans Barb Fish Cooker", 20, 20); //add text into rectangle
